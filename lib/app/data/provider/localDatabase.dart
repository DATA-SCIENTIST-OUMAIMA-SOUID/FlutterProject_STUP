...
class CartDatabase extends _$CartDatabase {
  final BehaviorSubject<List<CartProduct>> _cartProductsSubject =
      BehaviorSubject<List<CartProduct>>();
  CartDatabase()
  ...
  addProduct(ProductModel model, CartDatabase cartDatabase,
    ...
    allCartProducts.then((products) async {
      final bool productIsInList = products.any((product) =>
          product.id ==
          ("${model.id}~${model.variantInfo != null ? model.variantInfo!.variantId.toString() : ""}"));
      if (productIsInList) {
